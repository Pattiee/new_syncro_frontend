...
const Password = ({
  ...
  return (
    <>
      {/* Field label */}
      <label htmlFor={passwordHintId} className="block pb-2 text-gray-600 dark:text-gray-300">
        {name}
      </label>
      ...
      <div className="relative">
        <input
          ...
          minLength={8}
          maxLength={20}
          className={`${disabled ? 'bg-gray-100 dark:bg-gray-500' : 'bg-white dark:bg-gray-700'}
      ...
      {passwordValue && (passwordValue.length < 8 || confirmingPassword) && (
        <p
          className={`${confirmingPassword && passwordMatching ? 'text-green-500' : 'text-red-500'} text-base`}
        >
          {errMessage}
        </p>
      )}
