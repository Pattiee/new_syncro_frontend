...
// 1. Define explicit structure contracts for dynamic navigation links
...
interface RoleMenu {
  id: "vendor" | "manager" | "ceo";
  label: string;
  links: NavLinkItem[];
}

// Assumes root layout theme slice typings
type RootStateWithTheme = { theme?: "light" | "dark"; [key: string]: any };

const Navbar: React.FC = () => {
  const user = useSelector(selectCurrentUser);
  const { isEmpty } = useCart();
  const { pathname } = useLocation();
  const theme = useSelector((state: RootStateWithTheme) => state?.theme || "light");

  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState<boolean>(false);
  const [showNavbar, setShowNavbar] = useState<boolean>(true);
  const [lastScrollY, setLastScrollY] = useState<number>(0);
  const [dropdownOpen, setDropdownOpen] = useState<RoleMenu["id"] | null>(null);
  
  // Explicitly type the HTML div reference for layout listeners
  const dropdownRef = useRef<HTMLDivElement | null>(null);
  const [publicLinks] = useState<NavLinkItem[]>([]);

  // Scroll hide/show handler logic
  useEffect(() => {
    const handleScroll = () => {
  ...
  // Filter role-based links mapping utilizing type assertions
  const getLinksByRole = (links: unknown): NavLinkItem[] => {
    if (!user) return [];
    
    // Safety check fallback to verify structure before running filter passes
    const typedLinks = links as NavLinkItem[];
    const userRoles = user.roles || [];
    
    return typedLinks.filter((link) =>
  ...
  const renderDropdown = (menu: RoleMenu) => (
    <div
      key={menu.id}
